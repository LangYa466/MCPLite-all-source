...
extends EntityLivingBase {
    ...
    public void attackTargetEntityWithCurrentItem(Entity targetEntity) {
        float yaw;
        float f = yaw = RotationSetter.targetRotation != null ? RotationSetter.packetRotation.x : this.rotationYaw;
        if (targetEntity.canAttackWithItem() && !targetEntity.hitByEntity(this)) {
            ...
            if (f2 > 0.0f || f1 > 0.0f) {
                ...
                if (flag2) {
                    ...
                    if (i > 0) {
                        targetEntity.addVelocity(-MathHelper.sin(yaw * (float)Math.PI / 180.0f) * (float)i * 0.5f, 0.1, MathHelper.cos(yaw * (float)Math.PI / 180.0f) * (float)i * 0.5f);
                        this.motionX *= 0.6;
                        this.motionZ *= 0.6;
