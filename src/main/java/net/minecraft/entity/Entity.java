...
implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z) {
        ...
        if (isPlayer) {
            ...
            Client.eventManager.onMove(moveEvent);
    ...
    public boolean isInLava() {
        return this.worldObj.isMaterialInBB(this.getEntityBoundingBox().expand(-0.1f, -0.4f, -0.1f), Material.lava) && !NoLiquid.INSTANCE.getState();
    }

    public void moveFlying(double strafe, double forward, double friction) {
        StrafeEvent event = new StrafeEvent(this.rotationYaw);
        Client.eventManager.onStrafe(event);
        double yaw = event.yaw;
        double f = strafe * strafe + forward * forward;
        if (f >= (double)1.0E-4f) {
            if ((f = (double)MathHelper.sqrt_double(f)) < 1.0) {
                f = 1.0;
            }
            f = friction / f;
            double f1 = Math.sin(yaw * Math.PI / 180.0);
            double f2 = Math.cos(yaw * Math.PI / 180.0);
    ...
    public double getDistanceToEntityBox(AxisAlignedBB bb) {
        return this.getPositionEyes(1.0f).distanceTo(RotationUtils.getNearestPointBB(this.getPositionEyes(1.0f), bb));
    }
    ...
    public Vec3 getLook(float partialTicks) {
        Vector2f vector2f;
        Vector2f vector2f2 = vector2f = RotationSetter.targetRotation != null && this.getEntityId() == Minecraft.getMinecraft().getRenderViewEntity().getEntityId() ? RotationSetter.targetRotation : new Vector2f(this.rotationYaw, this.rotationPitch);
        if (partialTicks == 1.0f) {
            return this.getVectorForRotation(vector2f.y, vector2f.x);
        }
        float f = this.prevRotationPitch + (vector2f.y - this.prevRotationPitch) * partialTicks;
        float f1 = this.prevRotationYaw + (vector2f.x - this.prevRotationYaw) * partialTicks;
    ...
    public void writeToNBT(NBTTagCompound tagCompund) {
        try {
            ...
            tagCompund.setTag("RotationSetter", this.newFloatNBTList(this.rotationYaw, this.rotationPitch));
    ...
    public void readFromNBT(NBTTagCompound tagCompund) {
        try {
            ...
            NBTTagList nbttaglist2 = tagCompund.getTagList("RotationSetter", 5);
