...
implements INetHandlerPlayClient {
    ...
    private final MSTimer deadTimer = new MSTimer();
    ...
    public void handleSpawnObject(S0EPacketSpawnObject packetIn) {
        AntiExploit antiExploit = (AntiExploit)Client.moduleManager.moduleMap.get(AntiExploit.class);
        int type = packetIn.getType();
        if (antiExploit != null && antiExploit.getState() && type == 60) {
            int arrows = antiExploit.arrowMax;
            if (++arrows >= 100) {
                type = -1;
            }
        }
        ...
        if (entity != null) {
            ...
            if (packetIn.func_149009_m() > 0) {
                Entity entity2;
                if (type == 60 && (entity2 = this.clientWorldController.getEntityByID(packetIn.func_149009_m())) instanceof EntityLivingBase && entity instanceof EntityArrow) {
    ...
    public void handleSpawnExperienceOrb(S11PacketSpawnExperienceOrb packetIn) {
        AntiExploit antiExploit = (AntiExploit)Client.moduleManager.moduleMap.get(AntiExploit.class);
        boolean flag = false;
        flag = antiExploit != null && antiExploit.getState() ? packetIn.getXPValue() < 1000 : true;
        if (flag) {
            PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
    ...
    public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn) {
        ...
        if (Disabler.INSTANCE.fuck || entityplayer.isDead || entityplayer.getHealth() == 0.0f) {
            this.deadTimer.reset();
        }
        if (!NoRotSet.INSTANCE.getState() || entityplayer.isDead || entityplayer.getHealth() == 0.0f || Disabler.INSTANCE.fuck || !this.deadTimer.hasPassed(5000L)) {
            ...
            float yaw = RotationSetter.targetRotation != null ? RotationSetter.targetRotation.x : entityplayer.rotationYaw;
            float pitch = RotationSetter.targetRotation != null ? RotationSetter.targetRotation.y : entityplayer.rotationPitch;
    ...
    public void handleRespawn(S07PacketRespawn packetIn) {
        ...
        this.gameController.playerController.setGameType(packetIn.getGameType());
        Client.eventManager.onRespawn(new RespawnEvent());
    }

    @Override
    public void handleExplosion(S27PacketExplosion packetIn) {
        AntiExploit antiExploit = (AntiExploit)Client.moduleManager.moduleMap.get(AntiExploit.class);
        float strength = packetIn.getStrength();
        float func_149149_c = packetIn.func_149149_c();
        float func_149144_d = packetIn.func_149144_d();
        float func_149147_e = packetIn.func_149147_e();
        if (antiExploit != null && antiExploit.getState() && !antiExploit.grimAC) {
            float fixedStrength = MathHelper.clamp_float(func_149149_c, -1000.0f, 1000.0f);
            if (fixedStrength != strength) {
                strength = fixedStrength;
            }
            if (func_149149_c > 1.0f) {
                func_149149_c = 0.0f;
            }
            if (func_149144_d > 1.0f) {
                func_149144_d = 0.0f;
            }
            if (func_149147_e > 1.0f) {
                func_149147_e = 0.0f;
            }
        }
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        Explosion explosion = new Explosion(this.gameController.theWorld, null, packetIn.getX(), packetIn.getY(), packetIn.getZ(), strength, packetIn.getAffectedBlockPositions());
        explosion.doExplosionB(true);
        this.gameController.thePlayer.motionX += (double)func_149149_c;
        this.gameController.thePlayer.motionY += (double)func_149144_d;
        this.gameController.thePlayer.motionZ += (double)func_149147_e;
    }
    ...
    public void handleWindowItems(S30PacketWindowItems packetIn) {
        AntiExploit antiExploit = (AntiExploit)Client.moduleManager.moduleMap.get(AntiExploit.class);
        boolean flag = antiExploit != null && antiExploit.getState() ? Arrays.stream(packetIn.getItemStacks()).noneMatch(Objects::isNull) : true;
        if (flag) {
            PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
            EntityPlayerSP entityplayer = this.gameController.thePlayer;
    ...
    public void handleResourcePack(S48PacketResourcePackSend packetIn) {
        ...
        boolean flag = antiExploit != null && antiExploit.getState() ? !s.contains("log") && !s.contains("save") && !s.contains("option") && !s.contains("server") && !s.contains("user") : true;
        if (s.startsWith("level://") && flag) {
    ...
    public void handleParticles(S2APacketParticles packetIn) {
        ...
        if (antiExploit != null && antiExploit.getState()) {
            if (particleCount >= 500) {
                particleCount = 100;
            }
            if (particleSpeed >= 10.0f) {
                particleSpeed = 5.0f;
            }
        }
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        if (particleCount == 0) {
            double d0 = particleSpeed * packetIn.getXOffset();
            double d2 = particleSpeed * packetIn.getYOffset();
            double d4 = particleSpeed * packetIn.getZOffset();
            try {
