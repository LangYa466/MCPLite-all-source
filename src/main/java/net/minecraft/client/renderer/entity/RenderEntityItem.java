...
extends Render<EntityItem> {
    ...
    public int pyz(EntityItem itemIn, double x, double y, double z, float p_177077_8_, IBakedModel p_177077_9_) {
        float pitch;
        ItemStack itemstack = itemIn.getEntityItem();
        Item item = itemstack.getItem();
        ...
        if (!(item instanceof ItemBlock)) {
            GlStateManager.translate((double)((float)x), (double)((float)y) + 0.1, (double)((float)z));
        } else {
            GlStateManager.translate((double)((float)x), (double)((float)y) + 0.2, (double)((float)z));
        }
    ...
    private int pyzcs(EntityItem itemIn, double p_177077_2_, double p_177077_4_, double p_177077_6_, float p_177077_8_, IBakedModel p_177077_9_) {
        Minecraft mc = Minecraft.getMinecraft();
        boolean itemPhysics = Client.moduleManager.moduleMap.get(ItemDrop.class).getState();
        ItemStack itemstack = itemIn.getEntityItem();
        Item item = itemstack.getItem();
        if (item == null) {
            return 0;
        ...
        if (flag || this.renderManager.options != null) {
            ...
            if (itemPhysics) {
                if (itemIn.onGround) {
                    double var2 = (itemIn.posX + itemIn.motionX * (double)mc.timer.renderPartialTicks) * 200.0 + (itemIn.posZ + itemIn.motionZ * (double)mc.timer.renderPartialTicks) * 200.0;
                    GlStateManager.rotate((float)var2, 0.0f, 1.0f, 0.0f);
    ...
    public void doRender(EntityItem entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        int i = 0;
        ItemDrop itemDrop = (ItemDrop)Client.moduleManager.moduleMap.get(ItemDrop.class);
        i = itemDrop.getState() ? this.pyzcs(entity, x, y, z, partialTicks, ibakedmodel) : this.func_177077_a(entity, x, y, z, partialTicks, ibakedmodel);
        for (int j = 0; j < i; ++j) {
