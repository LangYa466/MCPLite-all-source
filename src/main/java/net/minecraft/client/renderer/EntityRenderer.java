...
implements IResourceManagerReloadListener {
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
            NoHurtCam noHurtCam = (NoHurtCam)Client.moduleManager.moduleMap.get(NoHurtCam.class);
    ...
    public void renderHand(float p_renderHand_1_, int p_renderHand_2_, boolean p_renderHand_3_, boolean p_renderHand_4_, boolean p_renderHand_5_) {
        if (!this.debugView) {
            ...
            if (p_renderHand_3_) {
                ...
                if (this.mc.gameSettings.thirdPersonView == 0 && !flag && !this.mc.playerController.isSpectator()) {
                    this.enableLightmap();
                    if (Config.isShaders()) {
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        if (this.renderHand && !Shaders.isShadowPass) {
            ...
            Render3DEventBeforeHand render3DEventBeforeHand = new Render3DEventBeforeHand(partialTicks);
            Client.eventManager.onRender3D(render3DEventBeforeHand);
            GlStateManager.clear(256);
            Render3DEventAfterHand render3DEventAfterHand = new Render3DEventAfterHand(partialTicks);
            Client.eventManager.onRender3D(render3DEventAfterHand);
            if (flag) {
