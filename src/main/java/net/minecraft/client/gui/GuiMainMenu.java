...
implements GuiYesNoCallback {
    ...
    private static final ResourceLocation[] newTitlePanoramaPaths = new ResourceLocation[]{new ResourceLocation("background/Night.png"), new ResourceLocation("client/EatPeople.jpg"), new ResourceLocation("client/whiteAndBlue.png"), new ResourceLocation("client/pools.png"), new ResourceLocation("background/Night5.png"), new ResourceLocation("background/Water.png"), new ResourceLocation("background/Sun.png")};
    ...
    public void initGui() {
        ...
        if (ClientSettings.INSTANCE.guiMainMenuStyles.equalsIgnoreCase("vanilla")) {
            int i = 24;
    ...
    private void renderSkybox(int p_73971_1_, int p_73971_2_, float p_73971_3_) {
        switch (ClientSettings.INSTANCE.guiMainMenuBackGround.toLowerCase()) {
            case "eatpeople": {
                RenderUtils.drawImage(newTitlePanoramaPaths[1], 0, 0, this.width, this.height);
                break;
            }
            case "mcphoto": {
                RenderUtils.drawImage(newTitlePanoramaPaths[0], 0, 0, this.width, this.height);
                break;
            }
            case "pools": {
                RenderUtils.drawImage(newTitlePanoramaPaths[3], 0, 0, this.width, this.height);
                break;
            }
            case "whiteandblue": {
                RenderUtils.drawImage(newTitlePanoramaPaths[2], 0, 0, this.width, this.height);
            }
        }
        if (!ClientSettings.INSTANCE.guiMainMenuBackGround.equalsIgnoreCase("vanilla")) {
            return;
        }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (ClientSettings.INSTANCE.guiMainMenuStyles.equalsIgnoreCase("mcplite")) {
            int addY = 10;
            int width = 252;
            int height = 25 + addY;
            int x = this.width / 2 - width / 2;
            int y = this.height / 2 - addY;
            Color bgcolor = new Color(0, 0, 0, 150);
            Color buttonColor = new Color(100, 100, 100, 50);
            RenderUtils.drawRoundedRect(x, y, width, height, 6.0f, bgcolor.getRGB(), 1.0f, bgcolor.getRGB());
            RenderUtils.drawRoundedRect(x, (float)y + (float)height / 2.0f - 10.0f, 3.0f, 20.0f, 1.0f, ClientColor.INSTANCE.getMixColor().getRGB(), 1.0f, ClientColor.INSTANCE.getMixColor().getRGB());
            y += addY / 2;
            for (int m = 0; m < 6; ++m) {
                float buttonWidth = (float)width / 6.0f;
                String[] icons = new String[]{"G", "H", "I", "K", "L", "C"};
                FontLoader.mainMenu36.drawString(icons[m], (float)x + buttonWidth * (float)m + 12.5f, y + 15, Color.white.getRGB());
                if (!this.isHover((int)((float)x + buttonWidth * (float)m), y, (int)buttonWidth, height, mouseX, mouseY)) continue;
            }
            String s1 = "MCPLite Hack Client 2.0";
            String s2 = "Minecraft 1.8.9 Copyright Mojang AB. Do not distribute!";
            this.fontRendererObj.drawString(s1, this.width - this.fontRendererObj.getStringWidth(s1), this.height - this.fontRendererObj.FONT_HEIGHT * 2, Color.white.getRGB());
            this.fontRendererObj.drawString(s2, this.width - this.fontRendererObj.getStringWidth(s2), this.height - this.fontRendererObj.FONT_HEIGHT, Color.white.getRGB());
            Date date = new Date();
            SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy MM dd");
            String s = "Date:" + dateFormat.format(date);
            this.fontRendererObj.drawString(s, 0, this.height - this.fontRendererObj.FONT_HEIGHT, Color.white.getRGB());
        }
        if (ClientSettings.INSTANCE.guiMainMenuStyles.equalsIgnoreCase("vanilla")) {
            if (l != 0 || i1 != 0) {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (ClientSettings.INSTANCE.guiMainMenuStyles.equalsIgnoreCase("mcplite")) {
            ...
            int x = this.width / 2 - width / 2;
            int y = this.height / 2;
            block11: for (int m = 0; m < 6; ++m) {
                float buttonWidth = (float)width / 6.0f;
                if (!this.isHover((int)((float)x + buttonWidth * (float)m), y, (int)buttonWidth, (int)((float)height / 2.0f), mouseX, mouseY)) continue;
                switch (m) {
                    case 0: {
                        this.mc.displayGuiScreen(new GuiSelectWorld(this));
                        continue block11;
