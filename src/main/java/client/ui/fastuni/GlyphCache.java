...
public class GlyphCache {
    ...
    void setDefaultFont(String name, int size, boolean antiAlias) {
        ...
        this.usedFonts.add(FontLoader.getSimpleFont(46));
        this.fontSize = size;
    ...
    Font lookupFont(char[] text, int start, int limit, int style) {
        Font font2;
        for (Font asdfas : this.usedFonts) {
            if (asdfas.canDisplayUpTo(text, start, limit) == start) continue;
            return asdfas.deriveFont(style, this.fontSize);
        }
        for (Font sf : this.allFonts) {
            if (sf.canDisplayUpTo(text, start, limit) == start) continue;
            this.usedFonts.add(sf);
            return sf.deriveFont(style, this.fontSize);
        }
