...
extends Gui {
    ...
    private final DecelerateAnimation textColor = new DecelerateAnimation(250, 1.0);
    private final DecelerateAnimation cursorBlinkAnimation = new DecelerateAnimation(750, 1.0);
    ...
    public void drawTextBox() {
        if (this.getVisible()) {
            ...
            if (this.isDrawingBackground()) {
                ...
                RenderUtils.drawRect2(this.xPosition, this.yPosition + this.height - rectHeight, this.width, rectHeight, ColorUtil.interpolateColor(this.focusedTextColor, this.unfocusedTextColor, this.textColor.getOutput().floatValue()));
            }
            this.textColor.setDirection(this.isFocused() ? Direction.BACKWARDS : Direction.FORWARDS);
            if (this.backgroundText != null) {
                Color backgroundTextColor = ColorUtil.applyOpacity(ColorUtil.applyOpacity(this.unfocusedTextColor, this.textAlpha), this.textColor.getOutput().floatValue());
                if (this.backgroundText.equals("Search")) {
                    FontLoaders.GoogleSans16.drawString("S", xPos + 1.5f, this.yPosition + FontLoaders.GoogleSans16.getMiddleOfBox(this.getHeight()), ColorUtil.applyOpacity(this.unfocusedTextColor, this.textAlpha).getRGB());
                    xPos += 15.0f;
                }
            ...
            boolean bl = cursorBlink = this.timerUtil.hasPassed(2000L) || cursorEndPos;
            if (canShowCursor) {
                if (cursorBlink) {
                ...
                RenderUtils.drawRect2(k1 + 1.0f, yPos - 2.0f, 0.5, this.font.getHeight() + 3, ColorUtil.applyOpacity(textColorWithAlpha, this.cursorBlinkAnimation.getOutput().floatValue()).getRGB());
            }
            if (selEnd != cursorPos) {
    ...
    public void setFont(CFontRenderer font) {
        this.font = font;
    }
